...
function FrontendInput({ register, index, control }) {
  const { fields, append, remove } = useFieldArray({
    control,
    name: `experiences[${index}].summary.Frontend`,
  });
  return (
    <div >
      <span className="font-extralight text-sm">FrontEnd experiences</span>
      ...
      {fields.map((item, indexFrontend) => (
        ...
        {...register(
              `experiences.${index}.summary.Frontend.${indexFrontend}`
            )}
          />
        </div>
      ))}
