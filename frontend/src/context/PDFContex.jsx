...
const PDFContext = ({ children }) => {
  ...
  const uploadImg = (selectedImage) => {
    if (selectedImage) {
      ...
      axios
        ...
        .then((res) => {
          let imgUrlArray = [];
          imgUrlArray.push(res.data.secure_url);
          dispatch({ type: "certification", certification: imgUrlArray });
        })
  ...
  const [debouncedValue, setValue] = useDebounceValue(defultValue, 3000);
  ...
  useEffect(() => {
    ...
    dispatch({
      ...
      contactInfo: {
        tel: debouncedValue.tel,
        email: debouncedValue.email,
        address: debouncedValue.address,
      },
    });

    dispatch({
      type: "experiencesChange",
